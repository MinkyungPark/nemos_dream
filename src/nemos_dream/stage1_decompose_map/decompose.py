...
def _decompose_data_designer(rows: list[RawInput]) -> list[DecomposeResult] | None:
    ...
    # (e.g. SK Telecom corp), that means DD silently cannot reach external NIM
    # endpoints and every request times out at ~80 s. Patch before DD imports
    # wire up their transports.
    from ._proxy_patch import apply_proxy_patches
    apply_proxy_patches()
