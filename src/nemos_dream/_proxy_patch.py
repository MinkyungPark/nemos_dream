...
The SK Telecom corporate proxy (`http://10.40.21.71:3128`) is required to reach
external endpoints like `integrate.api.nvidia.com` from inside the corp network.
Without patching, Data Designer (httpx+custom-transport) fails at runtime while
plain `httpx.get(...)` and `openai` sync work via default httpx behavior.

Ported from ``data_pipeline_phase1/src/proxy_patch.py``. Import and call
``apply_proxy_patches()`` once before any library usage (typically at the top
of a stage runner, right after ``load_dotenv()``).
"""
