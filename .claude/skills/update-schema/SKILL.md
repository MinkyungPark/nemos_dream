...
2. Bump `nemos_dream.__schema_version__` in `src/nemos_dream/__init__.py`.
   Stage 2 already has an open escape hatch
   (`Stage2Output.translation_meta: dict`, plus `Persona.extra` /
   `Style.extra`) — use those for ad-hoc metadata rather than bumping the
   version, unless the signal is clearly going to be load-bearing long-term.
